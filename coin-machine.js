function COIN_Machine() {
  ...
  function buyItem(item) {
    const cost = buyingOptions.find(option => option.id === item).cost;
  ...
  function createTheCoinShop() {
    ...
    buyingOptions.forEach(option => {
      document.getElementById(option.id).addEventListener('click', function (event) {
        buyItem(this.id);
      });
    });
  }
  ...
  function createTheMachine() {
    ...
    `;
    document.getElementById('coin-machine').addEventListener('click', () => {
      openCoinMachine();
    });
    createTheCoinShop();
    document.getElementById('coin-shop-dialog-wrapper').addEventListener('click', () => {
      closeCoinMachine();
    });
